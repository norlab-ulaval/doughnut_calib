...
def generate_launch_description():
    ...
    icp_mapper_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            os.path.join(include_folder, 'husky_drive_icp_mapper.launch.py')
        ])
    )

    imu_and_wheel_odom_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            os.path.join(include_folder, 'husky_imu_and_wheel_odom.launch.py')
        ])
    )

    pcl_deskew_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            os.path.join(include_folder, 'husky_pcl_deskew.launch.py')
        ])
    )
