...
PATH_DATAFRAME_DIAMOND = "drive_datasets/results_multiple_terrain_dataframe/geom_limits_by_terrain_for_filtered_cleared_path_warthog_max_lin_speed_all_speed_all_terrain_steady_state_dataset.pkl"
PATH_ROBOT_PARAM = "robot_param.yaml"

SCATTER_PLOT_TO_GENERATE = {"Body_vel": True, "Wheels_vel": True}
HIST_PLOT_TO_GENERATE = {"Accel_x": True, "Accel_y": True, "Accel_yaw": False, "Accel_yaw_imu": False}
...
def plot_histogramme(ax,df,column_of_interest,transient_only_flag=True,nb_bins=30,x_lim=(0,0),densitybool=True):
    ...
    for i in range(nbr_terrains):
        ...
        for k in range(sum(axis_param.values())):
            ...
            if "Wheels_vel" in list_of_plot_to_do:
                ...
                axs[k,i].set_title(f"Wheels vel on {terrain}")
                axs[k,i].set_facecolor(COLOR_DICT[terrain])
                axs[k,i].set_ylabel("left_wheel speed [rad/s]")
                axs[k,i].set_xlabel("right wheel speed [rad/s]")
